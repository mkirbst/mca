...
\item Output Enable mit anliegen Low-Pegel.
\end{itemize}
Abschließend wird der mit Output Enalbe belegte Pin 18 auf einen Low-Pegel gesetzt.

\subsubsection{PWM-Ausgabe}
\label{sec:pwmOut}
Für die Konfiguration der PWM-Ausgabe wird die Funktion \texttt{setPwm12(int n, int ontime)} verwendet. Dabei repräsentieren die Parameter \texttt{n} und \texttt{ontime} den entsprechenden PWM-Kanal beginnend mit 0 sowie den Anteil des High-Pegel der PWM-Ausgabe innerhalb eines Zyklus von 0 bis 4095. Wird der Funktion für den Parameter \texttt{ontime} der Wert 4096 übergeben, so ist die Ausgabe des entsprechenden Kanals ein durchgehender High-Pegel, wie in Abbildung [GRAFIK 4095/4096] dargestellt.
...
Ausgehend von diesen Differenzwerten wurde eine Korrekturfunktion 
\begin{equation}
•f(t)=t+t*m
\end{equation}
ermittelt. Dabei ist $t$ der vom TSL2561 erfasste Messwert und $m$ ein gemittelter Anstieg der Differenzfunktion. Um diesen Korrekturanstieg zu bestimmen, wurden für alle Messpunktpaare aus den vorangegangen dargestellten Messreihen Teilanstiege bestimmt. Anschließend erfolgte eine Mittelwertbildung über alle Teilanstiege einer Messreihe. Die daraus resulti
